...
class MLP(nn.Module):
    def __init__(
        self, in_channels, hidden_channels=None, out_channels=None, drop=0.0
    ):
        super().__init__()
...
class Block(nn.Module):
    ...
    def forward(self, feats, coords, index_0, index_1, index_0_offsets, n_max):
        ...
        feats = self.attn(
            feats, coords, index_0, index_1, index_0_offsets, n_max
        )
...
class BasicLayer(nn.Module):
    def __init__(
        ...
        self.down = (
            TransitionDown(
                embed_channels, out_channels, down_ratio, down_num_sample
            )
            if down
            else None
        )
    ...
    def forward(self, feats, coords, offset):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            mask = torch.arange(k).unsqueeze(0).cuda() < counts_blk.unsqueeze(
                -1
            )
            ...
            mask = torch.arange(k).unsqueeze(
                0
            ).cuda() < new_counts_blk.unsqueeze(
                -1
            )  # [n, k]
            ...
            new_index_0 = new_p2v_map_blk.unsqueeze(-1).expand(-1, -1, k)[
                mask_mat
            ]
            new_index_1 = new_p2v_map_blk.unsqueeze(1).expand(-1, k, -1)[
                mask_mat
            ]
        ...
        if self.down:
            feats_down, coords_down, offset_down = self.down(
                feats, coords, offset
            )
        else:
...
class TransitionUp(nn.Module):
    ...
    def forward(
        self, feats, coords, offset, skip_feats, skip_coords, skip_offset
    ):
...
class StratifiedTransformer(nn.Module):
    def __init__(
        ...
        assert (
            tp is not None
        ), "Please make sure torch_points_kernels is installed"
        ...
        for i in range(num_layers):
            layer = BasicLayer(
                ...
                out_channels=(
                    channels[i + 2] if i < num_layers - 1 else channels[i + 1]
                ),
                depth=depths[i],
