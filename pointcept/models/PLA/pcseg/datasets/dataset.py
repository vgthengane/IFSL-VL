...
class DatasetTemplate(torch_data.Dataset):
    def __init__(
        ...
        self.root_path = (
            ...
            else Path(self.dataset_cfg.DATA_PATH)
        )
        self.logger = logger
        ...
        if self.oss_client is not None:
            ...
            if dataset_cfg.get("VERSION", None):
                self.oss_root_path = os.path.join(
                    self.oss_root_path, dataset_cfg.VERSION
                )
        ...
        self.need_scene_caption = (
            hasattr(self, "caption_cfg")
            and self.caption_cfg.get("SCENE", None)
            and self.caption_cfg.SCENE.ENABLED
        )
        ...
        self.need_entity_caption = (
            hasattr(self, "caption_cfg")
            and self.caption_cfg.get("ENTITY", None)
            and self.caption_cfg.ENTITY.ENABLED
        )

        # KD keywords
        if self.training:
            self.load_kd_label = self.dataset_cfg.get(
                "LOAD_KD_LABEL_TRAIN", None
            )
        else:
            self.load_kd_label = self.dataset_cfg.get(
                "LOAD_KD_LABEL_TEST", None
            )
    ...
    @staticmethod
    def build_binary_class_mapper(
        base_class_idx, novel_class_idx, ignore_class_idx, ignore_idx
    ):
        remapper = (
            np.ones(256, dtype=np.int64) * ignore_idx
        )  # base: 1, novel: 0
    ...
    def collate_batch_indoor(self, batch_list):
        ...
        for key, val in data_dict.items():
            if key in ["points"]:
                ...
                for k in val[0]:
                    ret[key][k] = {}
                    ret[key][k]["idx"] = [
                        val[n][k]["idx"] for n in range(len(val))
                    ]
                    ret[key][k]["caption"] = []
                ...
                ret[key] = np.array(
                    [j for i in data_dict[key] for j in i], dtype=np.int32
                )
            else:
        ...
        ret["batch_idxs"] = ret["points_xyz_voxel_scale"][:, 0].astype(
            np.int32
        )
        if len(batch_list) == 1:
            ret["offsets"] = np.array([0, ret["batch_idxs"].shape[0]]).astype(
                np.int32
            )
        else:
            ret["offsets"] = np.cumsum(
                np.bincount(ret["batch_idxs"] + 1).astype(np.int32)
            )
    ...
    def collate_batch_lai(self, batch_list):
        ...
        for i in range(batch_size):
            data_dict["v2p_map"][i] = (
                accumulate_points_num + data_dict["v2p_map"][i]
            )
        ...
        if self.training:
            ret["voxel_coords"][:, 1:] += (torch.rand(3) * 2).type_as(
                ret["voxel_coords"]
            )
    ...
    def collate_batch_outdoor(batch_list):
        ...
        ret["offsets"] = np.cumsum(
            np.bincount((ret["points"][:, 0] + 1).astype(np.int32)).astype(
                np.int32
            )
        )
        ret["batch_size"] = batch_size
        ...
        for key in caption_cfg:
            if key in self.caption_keys and caption_cfg[key].ENABLED:
                caption_path = os.path.join(
                    data_path, caption_cfg[key].CAPTION_PATH
                )
                if self.oss_client:
                    caption_items[key.lower()] = copy.deepcopy(
                        json.load(self.oss_client.get(caption_path))
                    )
                else:
                    caption_items[key.lower()] = copy.deepcopy(
                        json.load(open(caption_path, "r"))
                    )
        return caption_items

    def select_caption_and_idx_all(
        self, scene_name, image_name_dict, image_corr_dict
    ):
    ...
    @staticmethod
    def select_caption_and_idx(
        caption, caption_cfg, scene_name, image_names, image_corr_indices
    ):
        if image_corr_indices is None:
            ...
            select_image_names, select_image_corr = (
                DatasetTemplate.select_images(
                    caption_cfg, image_names, image_corr_indices
                )
            )  # list (B, K), (B, K, N)
            # (B*K)
            select_captions = [
                caption[scene_name][n] for n in select_image_names
            ]
    ...
    def select_images(caption_cfg, image_name, image_corr):
        ...
        if caption_cfg.get("SAMPLE", 1) > 1:
            ...
            image_name = (
                np.array(image_name)[random_start :: caption_cfg.SAMPLE]
            ).tolist()
            image_corr = (
                np.array(image_corr, dtype=object)[
                    random_start :: caption_cfg.SAMPLE
                ]
            ).tolist()
        ...
        if image_name is None or len(image_name) == 0:  # lack 2d data
            ...
            selected_idx = np.random.choice(
                len(image_name),
                max(1, int(len(image_name) * ratio)),
                replace=False,
            )
        elif caption_cfg.SELECT == "hybrid":
            num = min(
                int(caption_cfg.NUM), int(len(image_name) * caption_cfg.RATIO)
            )
            selected_idx = np.random.choice(
                len(image_name),
                min(max(1, num), len(image_name)),
                replace=False,
            )
            ...
            if len(ratio_list) == 2:
                image_idx_sources.append(
                    [
                        ...
                        if not image_name[i].startswith("app")
                    ]
                )
                image_idx_sources.append(
                    [
                        i
                        for i in range(len(image_name))
                        if image_name[i].startswith("app")
                    ]
                )
                ...
                for jj in range(len(ratio_list) - 1):
                    image_idx_jj = [
                        i
                        for i in range(len(image_name))
                        if image_name[i].startswith(f"app_{jj}")
                    ]
                    image_idx_sources.append(image_idx_jj)
            assert len(image_idx_sources) == len(ratio_list)

            sample_ratios = []
            for image_idx_source, desired_ratio in zip(
                image_idx_sources, ratio_list
            ):
                sample_ratios.append(
                    desired_ratio / (len(image_idx_source) + 1e-6)
                )
            sample_ratios = (
                np.array(sample_ratios) / (max(sample_ratios) + 1e-6) * ratio
            )
            ...
            for ii in range(len(sample_ratios)):
                ...
                selected_idx_ii = np.random.choice(
                    ...
                    max(
                        1, int(len(image_idx_sources[ii]) * sample_ratios[ii])
                    ),
                    replace=False,
                )
        ...
        if selected_idx is not None:
            ...
            selected_image_corr = np.array(image_corr, dtype=object)[
                selected_idx
            ].tolist()
        else:
            selected_image_name = []
    ...
    def set_pseudo_labels_dir(
        self, pseudo_label_dir, use_pseudo_gt_label=False
    ):
        if os.path.exists(pseudo_label_dir):
            ...
            raise ValueError(
                "pseudo label path {} doesn't exist.".format(pseudo_label_dir)
            )

    def load_pseudo_labels(
        self, data_name, format="npy", dtype=np.int64, shape=(-1)
    ):
        # with open(str(self.pseudo_labels_dir / (data_name + '.npy')), 'r') as fin:
        labels = (
            np.load(
                str(
                    self.pseudo_label_dir / format / (data_name + f".{format}")
                )
            )
            .reshape(shape)
            .astype(dtype)
    ...
    def filter_points_with_n_captions(image_corr_dict, n_points, n_captions):
        n_captions_points = caption_utils.n_captions_for_points(
            image_corr_dict, n_points
        )
    ...
    def include_point_caption_idx(self):
        if self.need_view_caption and self.caption_cfg.VIEW.get(
            "IMAGE_CORR_PATH", None
        ):
            corr_path = self.caption_cfg.VIEW.IMAGE_CORR_PATH
            corr_path = (
                os.path.join(self.oss_root_path, corr_path)
                if self.oss_client
                else self.root_path / corr_path
            )
            point_caption_idx = (
                pickle.load(self.oss_client.get(corr_path))
                if self.oss_client
                else pickle.load(open(corr_path, "rb"))
            )
        else:
            point_caption_idx = None

        if self.need_entity_caption and self.caption_cfg.ENTITY.get(
            "IMAGE_CORR_PATH", None
        ):
