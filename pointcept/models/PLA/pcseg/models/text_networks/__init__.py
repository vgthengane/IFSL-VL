...
def load_text_embedding_from_path(text_emb_cfg, logger):
    if common_utils.oss_data_client is not None:
        text_emb_path = os.path.join(
            cfg.DATA_CONFIG.OSS_PATH, text_emb_cfg.PATH
        )
    ...
    logger.info(
        "=> loaded text embedding from path '{}'".format(text_emb_path)
    )
...
def build_text_token_from_class_names(model_cfg, class_names):
    if (
        model_cfg.TEMPLATE == "lseg"
    ):  # only instance classes are encoded with prompt
        return [
            (
                template_meta[model_cfg.TEMPLATE][0].format(c)
                if not is_bg_class(c)
                else c
            )
        ...
        return [
            template_meta[model_cfg.TEMPLATE][0].format(c) for c in class_names
        ]


def load_text_embedding_from_encoder(
    model_cfg, text_encoder, logger=logging.getLogger()
):
    text_encoder.cuda()
