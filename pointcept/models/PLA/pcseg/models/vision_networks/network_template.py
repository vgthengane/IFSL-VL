...
class ModelTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.num_class = len(
            num_class
        )  # change by zhaochong: here passing the all the class_names instead len(class_names) to avoid creating extra dataset object
        self.class_names = num_class

        self.dataset = dataset  # change by zhaochong: here passing the dataset_conf to avoid creating extra dataset object

        self.register_buffer("global_step", torch.LongTensor(1).zero_())

        self.fixed_modules = model_cfg.get("FIXED_MODULES", [])
        # For S3DIS
        self.test_x4_split = dataset.DATA_PROCESSOR.get("x4_split", False)
    ...
    def build_vfe(self, model_info_dict):
        ...
        vfe_module = vfe.__all__[self.model_cfg.VFE.NAME](
            ...
            num_point_features=self.dataset.get("NUM_POINT_FEATURES", None),
            voxel_size=self.dataset.get("VOXEL_SIZE", None),
            voxel_mode=self.dataset.DATA_PROCESSOR.get("voxel_mode", None),
        )
    ...
    def build_backbone_3d(self, model_info_dict):
        ...
        backbone3d_module = vision_backbones_3d.__all__[
            self.model_cfg.BACKBONE_3D.NAME
        ](model_cfg=self.model_cfg.BACKBONE_3D)
        model_info_dict["backbone3d_out_channel"] = (
            self.model_cfg.BACKBONE_3D.MID_CHANNEL
        )
    ...
    def load_params_from_file(
        self, filename, logger, epoch_id=None, to_cpu=False
    ):
    ...
    def remap_keys_from_3dlang(model_state_disk):
        ...
        for key in model_state_disk.keys():
            ...
            if first_name in ["input_conv", "unet", "output_layer"]:
                ...
                new_key = (
                    "binary_head.binary_classifier" + key[len(first_name) :]
                )
            elif first_name == "blinear_final":
                new_key = (
                    "binary_head.binary_classifier.2" + key[len(first_name) :]
                )
            elif first_name == "adapter":
                new_key = "adapter." + key
            elif (
                first_name == "tiny_unet"
                or first_name == "tiny_unet_outputlayer"
                or first_name == "mask_linear"
                or first_name == "iou_score_linear"
                or first_name == "offset_linear"
            ):
        ...
        for key in model_state_disk.keys():
            ...
            if first_name in ["module"]:
                if name_list[1] == "linear":
                    new_key = (
                        "task_head.cls_head"
                        + key[len(first_name) + len(name_list[1]) + 1 :]
                    )
                else:
