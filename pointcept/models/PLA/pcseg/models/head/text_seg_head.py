...
class TextSegHead(nn.Module):
    def __init__(self, model_cfg, in_channel, ignore_label, **kwargs):
        ...
        self.correct_seg_pred_binary = model_cfg.get(
        ...
        self.cls_head = nn.Linear(
            self.text_channel, self.num_class, bias=False
        )
        ...
        if self.need_class_mapping:
            self.idx_mapping = (
                category_remap_utils.cast_category_name_mapping_to_idx_mapping(
                    self.model_cfg.CLASS_MAPPING,
                    cfg.TEXT_ENCODER.CATEGORY_NAMES,
                    cfg.CLASS_NAMES,
                )
            )
        ...
        if self.seg_loss_weight is not None:
            self.seg_loss_weight = torch.FloatTensor(
                self.seg_loss_weight
            ).cuda()
            if hasattr(cfg.DATA_CONFIG, "base_class_idx"):
                self.seg_loss_weight = self.seg_loss_weight[
                    self.base_class_idx
                ]
            else:
                self.seg_loss_weight = self.seg_loss_weight[
    ...
    def set_cls_head_with_text_embed(self, text_embed):
        self.cls_head.load_state_dict(
            OrderedDict({"weight": text_embed.float()})
        )
    ...
    def forward(self, batch_dict):
        ...
        if self.no_v2p_map or (
            self.training and self.model_cfg.get("VOXEL_LOSS", None)
        ):
            pass
        else:
        ...
        if not self.training and self.need_class_mapping:
            semantic_scores = self.remap_category(
                semantic_scores, self.idx_mapping
            )
        ...
        if (
            ...
            binary_preds, semantic_preds = (
                self.correct_seg_pred_with_binary_pred(
        ...
        # changed by zhaochong, for aggregated features
        all_feats = adapter_feats[batch_dict["v2p_map"]]
        self.forward_ret_dict["all_feats"] = all_feats
        class_prototype = dict()
        class_similarity = dict()
        for class_id in semantic_preds.unique():
            class_mask = semantic_preds == class_id
            class_prototype[class_id.item()] = all_feats[class_mask].mean(
                dim=0
            )
            # class_similarity[class_id.item()] = torch.cosine_similarity(
            #     class_prototype[class_id.item()],
            #     self.cls_head.weight[class_id.item()],
            #     dim=0,
            # )
        self.forward_ret_dict["class_prototype"] = class_prototype
        # self.forward_ret_dict["class_similarity"] = class_similarity
        ...
        if (
            ...
            semantic_preds[~batch_dict["adapter_feats_mask"].bool().cuda()] = (
                self.ignore_label
            )
    ...
    def get_loss(self):
        ...
        seg_loss = self.seg_loss_func(
            semantic_scores, semantic_labels
        ) * self.model_cfg.get("LOSS_WEIGHT", 1.0)
    ...
    def correct_seg_pred_with_binary_pred(self, batch_dict, semantic_scores):
        ...
        base_semantic_scores = semantic_scores[
            ..., self.base_class_idx
        ].softmax(dim=-1)
        novel_semantic_scores = semantic_scores[
        ...
        sigmoid_binary_scores = sigmoid_binary_scores.repeat(
            1, semantic_scores.shape[-1]
        )
    ...
    def remap_category(semantic_scores, idx_mapping):
        new_semantic_scores = torch.zeros(
            (semantic_scores.shape[0], len(cfg.CLASS_NAMES)),
            dtype=torch.float32,
        ).cuda()
        for idx in range(new_semantic_scores.shape[1]):
            if isinstance(idx_mapping[idx], list):
                source_idxs = torch.from_numpy(
                    np.array(idx_mapping[idx], dtype=np.int64)
                ).cuda()
                ...
                new_semantic_scores[..., idx] = semantic_scores[
                    torch.arange(semantic_scores.shape[0]).cuda(),
                    max_score_idx,
                ]
            else:
                new_semantic_scores[..., idx] = semantic_scores[
