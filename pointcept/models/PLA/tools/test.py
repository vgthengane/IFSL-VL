...
def parse_config():
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        help="coords, offset, semantic, instance",
    )
    parser.add_argument(
        "--infer_time",
        action="store_true",
    ...
    parser.add_argument(
        "--oss_data", action="store_true", default=False, help=""
    )

    parser.add_argument(
        "--count_point_caption_num",
        action="store_true",
        default=False,
...
def eval_single_ckpt(
    ...
    # load checkpoint
    if args.ckpt:
        epoch_id = model.load_params_from_file(
            filename=args.ckpt,
            logger=logger,
            epoch_id=epoch_id,
            to_cpu=dist_test,
        ...
        logger.info("Warning: No checkpoint is loaded.")

    if text_embed is not None:
        model.task_head.set_cls_head_with_text_embed(text_embed)

    if epoch_id == "no_number":
        epoch_id = -1
    model.cuda()
...
def repeat_eval_ckpt(
    ...
    while True:
        ...
        if cur_epoch_id == -1 or int(float(cur_epoch_id)) < args.start_epoch:
            ...
            total_time += 30
            if total_time > args.max_waiting_mins * 60 and (
                first_eval is False
            ):
                break
            continue
...
def main():
    ...
    if args.batch_size is None:
        args.batch_size = cfg.OPTIMIZATION.get(
            "TEST_BATCH_SIZE_PER_GPU", cfg.OPTIMIZATION.BATCH_SIZE_PER_GPU
        )
    else:
        assert (
    ...
    output_dir = (
        cfg.ROOT_DIR / "output" / cfg.EXP_GROUP_PATH / cfg.TAG / args.extra_tag
    )
    ...
    if not args.eval_all:
        ...
        epoch_id = int(num_list[-1]) if num_list.__len__() > 0 else "no_number"
        eval_output_dir = (
            eval_output_dir
            / ("epoch_%s" % epoch_id)
            / cfg.DATA_CONFIG.DATA_SPLIT["test"]
    ...
    if args.oss_data or (cfg.get("OSS", None) and cfg.OSS.DATA):
        ...
        logger.info(
            f"Ceph client initialization with root path at {cfg.DATA_CONFIG.OSS_PATH}"
        )
    ...
    if getattr(args, "count_point_caption_num", False) or getattr(
        args, "count_caption_ratio_only", False
    ):
