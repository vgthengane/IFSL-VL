...
class SemSegTester(TesterBase):
    def test(self):
        ...
        if (
            ...
            with open(
                os.path.join(save_path, "submit", "test", "submission.json"),
                "w",
            ) as f:
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            if (
                ...
                np.savetxt(
                    os.path.join(
                        save_path, "submit", "{}.txt".format(data_name)
                    ),
...
@TESTERS.register_module()
class GFSSemSegTester(TesterBase):
    def __init__(
        self,
        cfg,
        model=None,
    ...
    def test(self):
        mean_mIoUs = np.zeros(4)
        mean_iou_list = []

        result = self.one_regis_test()
        if comm.is_main_process():
            mIoU, base_iou, novel_iou, hm_iou, iou_list = result
        ...
        if comm.is_main_process():
            ...
            return mIoU, base_iou, novel_iou, hm_iou, iou_list
        else:
            return None
    ...
    def one_regis_test(self, **kwargs):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            pred = torch.zeros(
                (segment.size, self.cfg.data.num_base_novels)
            ).cuda()
            ...
            for i in range(len(fragment_list)):
                ...
                idx_part = input_dict["index"]
                with torch.no_grad():
                    pred_part = self.model(input_dict, **kwargs)[
                        "seg_logits"
                    ]  # (n, k)
                    pred_part = F.softmax(pred_part, -1)
            ...
            if self.cfg.vis is not None:
                save_dir = os.path.join(self.cfg.vis, data_name)
                os.makedirs(save_dir, exist_ok=True)
                np.save(
                    os.path.join(save_dir, "query.npy"),
                    np.concatenate(
                        [data_dict["coord"], data_dict["color"]], axis=-1
                    ),
                )
                np.save(
                    os.path.join(save_dir, "query_label.npy"),
                    segment,
                )
                np.save(
                    os.path.join(save_dir, "query_pred.npy"),
                    pred,
                )
        ...
        if comm.is_main_process():
            ...
            logger.info(
                "Val result: mIoU/Base/Novel/HM/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}/{:.4f}/{:.4f}/{:.4f}".format(
                    mIoU, base_iou, novel_iou, hm_iou, mAcc, allAcc
                )
            )
            ...
            logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
            # Prepare the results to be broadcasted
...
class PartSegTester(TesterBase):
    def test(self):
        ...
        iou_category, iou_count = np.zeros(num_categories), np.zeros(
            num_categories
        )
        self.model.eval()
        ...
        save_path = os.path.join(
            self.cfg.save_path,
            "result",
            "test_epoch{}".format(self.cfg.test_epoch),
        )
        make_dirs(save_path)
        ...
        for idx in range(len(test_dataset)):
            ...
            for i in range(batch_num):
                ...
                if self.cfg.empty_cache:
                    torch.cuda.empty_cache()
                pred_part = pred_part.reshape(
                    -1, label.size, self.cfg.data.num_classes
                )
