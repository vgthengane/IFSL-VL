...
class ClsEvaluator(HookBase):
    ...
    def eval(self):
        ...
        self.trainer.comm_info["current_metric_value"] = (
            all_acc  # save for saver
        )
        self.trainer.comm_info["current_metric_name"] = (
            "allAcc"  # save for saver
        )
...
class InsSegEvaluator(HookBase):
    ...
    def evaluate_matches(self, scenes):
        ...
        ap_table = np.zeros(
            (len(dist_threshes), len(self.valid_class_names), len(overlaps)),
            float,
        )
            ...
            for oi, overlap_th in enumerate(overlaps):
                ...
                for li, label_name in enumerate(self.valid_class_names):
                    ...
                    for scene in scenes:
                        ...
                        cur_score = np.ones(len(gt_instances)) * (
                            -float("inf")
                        )
...
@HOOKS.register_module()
class GFS_MultiregTrain_PreciseEvaluator(HookBase):
    def __init__(self, test_last=False):
        super().__init__()
        self.test_last = test_last
        self.mean_mIoUs = np.zeros(4)
        self.mean_iou_list = []
    ...
    def after_train(self):
        ...
        if comm.is_main_process():
            ...
            self.mean_mIoUs += np.array([mIoU, base_iou, novel_iou, hm_iou])
            self.mean_iou_list.append(iou_list)
