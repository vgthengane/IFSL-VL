...
class IterationTimer(HookBase):
    ...
    def before_train(self):
        self._start_time = time.perf_counter()
        self._remain_iter = self.trainer.max_epoch * len(
    ...
    def after_step(self):
        ...
        self._remain_iter -= 1
        remain_time = (
            self._remain_iter * self.trainer.storage.history("batch_time").avg
        )
...
class InformationWriter(HookBase):
    ...
    def before_train(self):
        ...
        self.curr_iter = self.trainer.start_epoch * len(
            self.trainer.train_loader
        )
...
class CheckpointSaver(HookBase):
    def __init__(self, save_freq=None):
        self.save_freq = (
            save_freq  # None or int, None indicate only save model last
        )
    ...
    def after_epoch(self):
        if is_main_process():
            ...
            if self.trainer.cfg.evaluate:
                current_metric_value = self.trainer.comm_info[
                    "current_metric_value"
                ]
                current_metric_name = self.trainer.comm_info[
                    "current_metric_name"
                ]
            ...
            if (
                self.save_freq
                and (self.trainer.epoch + 1) % self.save_freq == 0
            ):
                shutil.copyfile(
                    filename,
                    os.path.join(
...
@HOOKS.register_module()
class CheckpointSaver_MultiRegis(HookBase):
    def __init__(self, save_freq=None):
        self.save_freq = (
            save_freq  # None or int, None indicate only save model last
        )
    ...
    def after_epoch(self):
        if is_main_process():
            ...
            filename = os.path.join(
                self.trainer.cfg.save_path,
                "model",
                f"{self.trainer.regis_str}_model_last.pth",
            )
            self.trainer.logger.info("Saving checkpoint to: " + filename)
            torch.save(
                {
            ...
            if is_best:
                shutil.copyfile(
                    ...
                    os.path.join(
                        self.trainer.cfg.save_path,
                        "model",
                        f"{self.trainer.regis_str}_model_best.pth",
                    ),
                )
            if (
                self.save_freq
    ...
    def before_train(self):
        ...
        if self.trainer.cfg.weight and os.path.isfile(self.trainer.cfg.weight):
            ...
            if self.trainer.cfg.resume:
                ...
                self.trainer.best_metric_value = checkpoint[
                    "best_metric_value"
                ]
...
class BackboneLoader(HookBase):
    ...
    def before_train(self):
        self.trainer.logger.info("=> Loading checkpoint & weight ...")
        if self.trainer.cfg.backbone_weight and os.path.isfile(
            self.trainer.cfg.backbone_weight
        ):
            self.trainer.logger.info(
                f"Loading weight at: {self.trainer.cfg.backbone_weight}"
            )
            checkpoint = torch.load(
                self.trainer.cfg.backbone_weight,
                map_location=lambda storage, loc: storage.cuda(),
            ...
            # Determine the correct model reference for DDP or single GPU
            model = (
            ...
            self.trainer.logger.warning(
                f"Unexpected keys in weight: {load_state_info.unexpected_keys}"
            )

        elif self.trainer.cfg.backbone_weight and not os.path.isfile(
            self.trainer.cfg.backbone_weight
        ):
            raise Exception(
                f"No weight found at: {self.trainer.cfg.backbone_weight}"
            )
