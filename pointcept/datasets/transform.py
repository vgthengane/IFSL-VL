...
class ToTensor(object):
    def __call__(self, data):
        ...
        elif isinstance(data, np.ndarray) and np.issubdtype(
...
class RandomDropout(object):
    ...
    def __call__(self, data_dict):
        if random.random() < self.dropout_application_ratio:
            ...
            if "all_segment" in data_dict.keys():
                data_dict["all_segment"] = data_dict["all_segment"][idx]
            if "loss_mask" in data_dict.keys():
                data_dict["loss_mask"] = data_dict["loss_mask"][idx]
            if "mask" in data_dict.keys():
                data_dict["mask"] = data_dict["mask"][idx]
            if "zslabel" in data_dict.keys():
                data_dict["zslabel"] = data_dict["zslabel"][idx]
...
class RandomRotate(object):
    def __init__(
        self, angle=None, center=None, axis="z", always_apply=False, p=0.5
    ):
    ...
    def __call__(self, data_dict):
        ...
        if "coord" in data_dict.keys():
            ...
            data_dict["coord"] = np.dot(
                data_dict["coord"], np.transpose(rot_t)
            )
            data_dict["coord"] += center
        if "normal" in data_dict.keys():
            data_dict["normal"] = np.dot(
                data_dict["normal"], np.transpose(rot_t)
            )
        return data_dict
...
class ChromaticAutoContrast(object):
    ...
    def __call__(self, data_dict):
        if "color" in data_dict.keys() and np.random.rand() < self.p:
            ...
            data_dict["color"][:, :3] = (1 - blend_factor) * data_dict[
                "color"
            ][:, :3] + blend_factor * contrast_feat
        return data_dict
...
class RandomColorJitter(object):
    ...
    def adjust_brightness(self, color, brightness_factor):
        if brightness_factor < 0:
            raise ValueError(
                "brightness_factor ({}) is not non-negative.".format(
                    brightness_factor
                )
            )
    ...
    def adjust_contrast(self, color, contrast_factor):
        if contrast_factor < 0:
            raise ValueError(
                "contrast_factor ({}) is not non-negative.".format(
                    contrast_factor
                )
            )
    ...
    def adjust_saturation(self, color, saturation_factor):
        if saturation_factor < 0:
            raise ValueError(
                "saturation_factor ({}) is not non-negative.".format(
                    saturation_factor
                )
            )
    ...
    def get_params(brightness, contrast, saturation, hue):
        ...
        c = (
            None
            if contrast is None
            else np.random.uniform(contrast[0], contrast[1])
        )
        s = (
            None
    ...
    def __call__(self, data_dict):
        ...
        for fn_id in fn_idx:
            if (
                ...
                data_dict["color"] = self.adjust_hue(
                    data_dict["color"], hue_factor
                )
        return data_dict
...
class HueSaturationTranslation(object):
    ...
    def __call__(self, data_dict):
        if "color" in data_dict.keys():
            ...
            hsv = HueSaturationTranslation.rgb_to_hsv(
                data_dict["color"][:, :3]
            )
...
class ElasticDistortion(object):
    ...
    def elastic_distortion(coords, granularity, magnitude):
        ...
        noise_dim = ((coords - coords_min).max(0) // granularity).astype(
            int
        ) + 3
...
class GridSample(object):
    def __init__(
        ...
        self.hash = (
            self.fnv_hash_vec if hash_type == "fnv" else self.ravel_hash_vec
        )
    ...
    def __call__(self, data_dict):
        ...
        if self.mode == "train":  # train mode
            ...
            if "all_segment" in data_dict.keys():
                data_dict["all_segment"] = data_dict["all_segment"][idx_unique]
            if "loss_mask" in data_dict.keys():
                data_dict["loss_mask"] = data_dict["loss_mask"][idx_unique]
            if "mask" in data_dict.keys():
                data_dict["mask"] = data_dict["mask"][idx_unique]
            if "zslabel" in data_dict.keys():
                data_dict["zslabel"] = data_dict["zslabel"][idx_unique]
            return data_dict
            ...
            for i in range(count.max()):
                idx_select = (
                    np.cumsum(np.insert(count, 0, 0)[0:-1]) + i % count
                )
    ...
    def __call__(self, data_dict):
        ...
        if self.mode == "all":
            ...
            if data_dict["coord"].shape[0] > point_max:
                ...
                while idx_uni.size != data_dict["index"].shape[0]:
                    ...
                    dist2 = np.sum(
                        np.power(
                            data_dict["coord"] - data_dict["coord"][init_idx],
                            2,
                        ),
                        1,
                    )
                    ...
                    if "normal" in data_dict.keys():
                        data_crop_dict["normal"] = data_dict["normal"][
                            idx_crop
                        ]
                    if "color" in data_dict.keys():
                        data_crop_dict["color"] = data_dict["color"][idx_crop]
                    if "displacement" in data_dict.keys():
                        data_crop_dict["displacement"] = data_dict[
                            "displacement"
                        ][idx_crop]
                    if "strength" in data_dict.keys():
                        data_crop_dict["strength"] = data_dict["strength"][
                            idx_crop
                        ]
                    ...
                    delta = np.square(
                        1
                        - data_crop_dict["weight"]
                        / np.max(data_crop_dict["weight"])
                    )
                ...
                data_crop_dict["weight"] = np.zeros(
                    data_dict["coord"].shape[0]
                )
        ...
        # mode is "random" or "center"
            ...
            if self.mode == "random":
                ...
                raise NotImplementedError
            idx_crop = np.argsort(
                np.sum(np.square(data_dict["coord"] - center), 1)
            )[:point_max]
            if "coord" in data_dict.keys():
            ...
            if "all_segment" in data_dict.keys():
                data_dict["all_segment"] = data_dict["all_segment"][idx_crop]
            if "loss_mask" in data_dict.keys():
                data_dict["loss_mask"] = data_dict["loss_mask"][idx_crop]
            if "mask" in data_dict.keys():
                data_dict["mask"] = data_dict["mask"][idx_crop]
            if "zslabel" in data_dict.keys():
                data_dict["zslabel"] = data_dict["zslabel"][idx_crop]
            if "instance" in data_dict.keys():
...
class ShufflePoint(object):
    def __call__(self, data_dict):
        ...
        if "displacement" in data_dict.keys():
            data_dict["displacement"] = data_dict["displacement"][
                shuffle_index
            ]
        if "color" in data_dict.keys():
...
class InstanceParser(object):
    def __init__(
        self, segment_ignore_index=(-1, 0, 1), instance_ignore_index=-1
    ):
